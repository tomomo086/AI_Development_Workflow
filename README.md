# 🤖 未経験者が編み出したAI開発ワークフロー - 実践的開発環境

> エンジニア未経験からたった1年で実用レベルのAI統合開発環境を構築した実体験記録
...
- [⚡ このワークフローの「とがりポイント」](#-このワークフローのとがりポイント)
- [💡 特徴](#-特徴)
...
**「AIに舵を取られない」**：AIが教えてくれることを鵜呑みにせず、批判的思考で人間が主導権を握る。これが未経験者でも実用レベルにたどり着けた最重要ポイントです。

**重要**: AIは強力なツールですが、人間の判断と検証が不可欠です。この姿勢を貫くことで、AI活用の真の価値を引き出せます。
...
3. **ゲーマー視点のハードウェア選択**：趣味と実益の両立を実現した環境構築
4. **未経験者だからこその気づき**：専門家には見えない実践的な盲点を発見
...
- **Claude Desktop**
  - 💚 **個人的な発見**: 「チャットUIだから初心者でも安心」→ MCPの拡張性で本格開発も可能
  - 利点: チャットUIで視認性が高く直感的に対話可能／MCPによる拡張性で万能性あり／ZapierMCP連携でGoogleドライブ等にスマホからもアクセスしやすい
  - 💔 **つまずいた点**: 処理速度がやや遅い／ファイル操作の現状が分かりにくい／途中停止は強制ストップが必要

- **ClaudeCode**
  - 💚 **驚いた発見**: 「CLIは難しそう」→「途中で止まって確認してくれるから安心」
  - 利点: CLIベースで処理が速い／操作中のファイルが可視化される／途中で許可を求めてくるので進捗管理しやすい
  - 💔 **課題として感じた点**: Cursorほど細かな調整ができない／CLIは初心者には複雑／視覚的に分かりにくい／Claudeの性能に依存するためAIの得手不得手がそのまま反映される／単一AIに依存するため多様性に欠け、異なるAIの強みを活用できない

- **Cursor**
  - 💚 **救われた体験**: 「AIが暴走して大変だった経験」→「Reject/Acceptで制御できる発見」
  - 利点: Reject/Acceptによる細かな段階的修正／複数AIを使い分け可能／リアルタイムエラー検出や自動フォーマットなど細やかな品質向上が可能
  - 💔 **感じた限界**: ClaudeCodeほどのスピード感はない／AIの提案精度はプロンプトや文脈に依存する場合あり

### 🎯 なぜこの組み合わせにたどり着いたか
**失敗から学んだ教訓**: 単一ツールでは限界がある → 各ツールの強みを活かした「適材適所」戦略が、未経験者でも実用レベルを実現する秘訣
...
### 🎮 ゲーマー×開発者の両立設計
**個人的なこだわり**: 趣味であるオンラインゲームを快適に楽しみ、妻とも一緒にプレイできる性能を重視しつつ、開発用途とも両立できるバランスを意識しています。この「遊びと仕事の両立」が、長時間の開発作業を持続可能にする秘訣です。
